...
def generarPDF(estados, estados_aceptacion, transiciones, estado_inicial, nombre,afn):
    w, h = A4
    ...
    c.drawString(30, h - 70, "Nombre del AFN:  " + nombre)
    ...
    Cadena = Clases.AFN.generar_cadena_ejemplo(afn)
    print(Cadena)

    c.setFont('Helvetica', 12)

    # Ajustar las coordenadas para que las líneas de texto estén encima de la imagen
    c.drawString(30, image_y + image_height + 20, "------------------------------------------------------- Gráfica del AFN -------------------------------------------------------")
...
def generarReporteAFD(estados, estados_aceptacion, transiciones, transicionesN,estado_inicial, nombre,afn):
    print("test1")
    generarDOT(estados, estados_aceptacion, transiciones, estado_inicial,nombre)
    print("test2")
    generarPDF(estados, estados_aceptacion, transicionesN, estado_inicial, nombre,afn)
    return True
